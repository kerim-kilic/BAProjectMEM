...
provinces$Latitud <- ifelse(provinces$PROVINCIA == "STA CRUZ DE TENERIFE", 35.5, provinces$Latitud)
provinces$Longitud <- ifelse(provinces$PROVINCIA == "STA CRUZ DE TENERIFE", -11.3, provinces$Longitud)
...
ggplot(Spain) + geom_sf(fill = "#00CC77", color = "#999999") + 
  geom_point(data = Revenue_region, aes(x = Longitude, y = Latitude, size = Volume_per_capita))
```
