...
This documents aims to  help the company DDI to develop an adequate commercial strategy based on data, making specific marketing proposals for the different products. In order to do so, first the data will be cleaned. Following, a general overview of the data available will be shown. Then, a RFM analysis will take place, followed by a CLV one. After this, some association rules will be studied. Lastly, this project's conclusions will be presented. The code for this project can be found on the following GitHub repository: <a href="https://github.com/kerim-kilic/BAProjectMEM">"https://github.com/kerim-kilic/BAProjectMEM"</a>

# Software Setup

The software used for the development of the study and the writing of the report is R<strong>[1]</strong>. The following libraries are used to develop the code and perform the analysis:

* tidyverse <strong>[2]</strong>
* gridExtra <strong>[3]</strong>
* janitor <strong>[4]</strong>
* lubridate <strong>[5]</strong>
* arules <strong>[6]</strong>
* formattable <strong>[7]</strong>
* rfm <strong>[8]</strong>
* CLVTools <strong>[9]</strong>
* mapSpain <strong>[10]</strong>
* arulesViz <strong>[11]</strong>

# Data

The data used in this report is provided by DDI. There are in total four tables:
...
The table t_datallista, from now on referred as <strong>retailers</strong>. This table contains information about the retailers, such as: the id of the retailer, establishment, and customer; Geographical location, distributor, category, and the type of the establishment.

In addition, the table material_preu_damm, now referred as <strong>prices</strong>. This data includes the different products and their prices 

Then, there is the table t_material_Freqs, now called <strong>quantity</strong>. This data refers to the different products and their respective quantity.

Lastly, there is the table t_posiciones, now called <strong>orders</strong>. This data includes information about the orders placed by each establishment. It includes the ID of the establishment, the date the order was placed, as well as which material was ordered, in which unit of measure and its quantity. 
...
Checking the he "Unit" variable now:
```{r}
...
Each product is part of a product group that has multiple products from the same brand. These are the following product groups: 
...
To perform the analysis, the tables orders, and prices should be merged. This is done using the merge function, and doing it by the "material" column. To have relevant data, this data is filtered from 2002 onwards. 
...
Looking through the category data a lot of NA values can be seen, and after checking with the company, it was realized that they are not classified yet. Thus, they will be marked as so for now and classify them afterward as part of the work. 
...
There are only `r nrow(retailers[is.na(retailers$customer),])` customer values missing. Since it might be needed clean customer values for later merging with the orders table, it will just be removed the rows with NAs, as it will not affect the data anyhow.
...
There are many retailers for which the field **Region** has a value of '/ES'. However, further inspection has shown that in many cases the same establishment has a determined region in another entry. This fact was used to replace the undetermined values '/ES' with a valid value in other entries.

This was done by splitting the data frame into one part containing the values '/ES' in the region and another with the rest. Then, the known regions were combined with the unknown by using 'id_establishment'. Finally, both data frames were combined by using **rbind** function.
...
The method previously described worked for all the entries in which the value was '/ES'. Additionally, there were a few entries with the value '/'. These were simply deleted from the data set.
...
In this section of the report, a general overview of the data will be shown using graphs. First, is the evolution of order lines over years and months. Then, the top clients will be observed. Following, the most popular client locations will be analyzed, as well as the preferred product type. 
...
First, the number of times a specific article type appears in the dataset is counted, having like that the number of order lines referring to this type of product, Then, from this, a graph taking into account the percentage from the total number of order lines are made.  
...
It can be seen that this graph is aligned with the previous one. It shows that by far, the product which more revenue represents is Estrella, with a 66%. Then there is Turia, followed by Free and Lemon. In the lower part of the scale, Xibeca and Equilater are found. 
...
It can be seen that the revenue graph does not follow the same pattern as the number of order lines one. This is since one order line can represent a lot of revenue if the quantity of that order line is really big, or the price of the product is high. It should be noted the peak in revenue in 2018, is not seen in the number of order lines graph. 
...
It can be seen that the number of lines follow a cyclical pattern each year. It can be seen that the peaks are located in the month of January of every year, and the lowest points at the last quarter of the year. The red line shows the moment COVID-19 pandemic restrictions started in Spain. A clear decline in order lines can be seen after this. The peak in 2021 is much lower than the previous ones. 
...
Here again, it can be seen that the revenue peaks are in January. The 2018 revenue peak mentioned before, is seen again. After COVID, the revenue, as the order lines did, decreased. 

## Retailers order lines per location

It is interesting to study where the most order lines come from in terms of location. This will be analyzed now. 
...
It can be seen that more than 30% of the retailers are located in Barcelona, whereas less than 2% are in Vizcaya. 

## Geographical analysis

It was also considered of interest the geographical distribution of variables such as\ sales, expressed in terms of money and volume.

To perform this analysis, an external data set 'Provinces' with geographical and population information was imported. By using merge on several occasions, this kind of data was connected with the orders [12].
...
Aggregated and per capital fields were added to represent sales money and volume-wise for the different provinces of Spain.
...
the sales of Damm for every province. The map on the left represents the number of liters sold in *2020*. Additionally, in order to consider the effect of the population, on the right it is possible to see the representation of the average consumption of beer Damm in the same year. 
...
Barcelona is by far the province in which the biggest volume of Damm products is sold, with more than **24** million liters. In Tarragona, Madrid, and Palma around half a million liters are distributed.
...
The results are very similar to those of the sales expressed in volume with a couple of exceptions. Considering the total of sales, Gerona became more important than Palma in terms of money. Regarding the consumption per capita, Logroño has lower performance, going from second to the fourth position.

### Company vs market performance

In the previous section, the performance of Damm was analyzed in the provinces throughout Spain. However, it was considered interesting to revise the behavior of sales in comparison with the whole market. To do this, the spending of people on beer was investigated by external sources [13]. This information was represented in the map below. The community with the highest consumption was Islas Baleares, with **€39.4** on average in *2020*, probably boosted by tourism. On the other hand, in Asturias people spent **€21.3** on average for the same period. In Catalonia, in turn, this number was **€33**.
...
Regarding the performance of the company, it has the best results in Catalonia with sales of **8.5** euros per person per year. In Rioja, the outcome was **€7** and in Balearic Islands, it was **5.7**. On the other side, in 8 of the 17 Autonomous Communities, the consumption per capita was less than **€1** per year, that is, the presence of the companies in those territories is minor.
...
In this sense, the company has a great potential for growth in the Communities where so far is not a dominant player.


# RFM

Given the historical data about retailers and transactions, it is possible to perform an analysis of Recency, Frequency, and Money [spent]. To do this, a revenue column was added.

The *rfm* library was used. The classification is done with the function *rfm_table_order*. For the sake of the analysis, the final date was set at the end of 2020.
...
The following plots help represent the general distribution of the clients, according to the variables *frequency, recency and money*. For instance, the heat map shown below is useful by telling that most of the revenue is given by the customers who more frequently and most recently have purchased products of the company.
...
With the plot below it is possible to assert that the clients with the highest recency and frequency, tend to have a high monetary score. That is to say, in recent history, the most frequent clients, spend important amounts of money, which is good for the company.
...
When the whole data is considered, it is possible to see that the distribution of clients in the three variables tends to be asymmetrical, in which most of them do orders of low money, have ordered very recently and have a low frequency. 
This implies that a small bunch of customers has spent big amounts of money in comparison with the rest.
...
This can be validated by the scatter plot below. There are 5 clients that outperform the vast majority. It is observed a trend that says the more frequency, the more money spent.
...
```{r,fig.show='hide'}
## ----some scatter plots---------------------------------------------------------
p1 <- rfm_rm_plot(rfm_result)
p2 <- rfm_rf_plot(rfm_result)
```
...
For example, the plot below allows the abalysis of the distribution of money for the proposed categories. For example, clients of the type *Champion* have purchased with a median value of roughly 3.5 million euros. 
...
Also, when analyzing *frequency*, it is possible to observe that for *lost* and *about to sleep* clients, they just register a few orders. On the other side, *champions* typically have several hundred.
...
Several segments deserve special attention. For instance, *Need attention* or *Others*, had median expenditures of roughly half a million euros, but with recency days over 100. These are important types of clients that might be in danger. It is required to take care of them.

Additionally, the category *Potential Loyalist* has 67 customers, which had purchased 0.18 million as median, which is not negligible. However, the frequency is low, but they are recent. These are new clients that need attention to consolidate the commercial relation.

# CLV
The historical data, especially for the recent years, gives also the opportunity to perform the Customer Lifetime Value (CLV) analysis. CLV reflects the total worth (money spend)  by the purchasers during the whole period they were in a relationship with DAMM.

In order to have relevant results, as well as have continuity in customer relationships, the data was first filtered since January 2018 to June 2021.The estimation period was selected as the first three years (156 weeks), thus leaving half a year (the first five months of 2021) for the holdout period.
...
clv.transac <- clvdata(orders_prices_clv,
  date.format = "ymd",
  time.unit = "week",
...
Above is presented the summary of the transactions data from the customer lifetime perspective. While the table is self-explanatory, it is worth highlighting the *% zero repeaters*, which indicates that 12% did a purchase a single time in 3 years. This metric already leaves a point for a business follow-up and a further deep dive, combined with the RFM results above. This might highlighting specific customers to invest more for boosting up the lifetime value.


## The Estimation Pareto 
For the probabilistic attrition model the industry-standard Pareto/NBD model was used, and the model parameters were estimated through the pnbd() command accordingly.

```{r}
est.pnbd <- pnbd(clv.data = clv.transac)
est.pnbd
```

The model parameters are now estimated, and the predictions for the holdout period can now be calculated for individual customers. 
...
One example is presented in a row. The expanded table allows to review the up-to-date actual spendings for each customer, the probability of them being alive, and most importantly: predicted spending and the predicted customer lifetime value. 

```{r}
pred_holdout <- filter(pred_holdout, pred_holdout$actual.total.spending < 5000000)
options(scipen=5)
plot(x=pred_holdout$actual.x, y=pred_holdout$actual.total.spending,
     pch = 23,
     col = 3,
     xlab = "Number of actual transactions",
     ylab = "Toal spending",
     main = "Correlation between # of transactions and customer total spending")
```

Evidently, there are customers, which purchase frequently (more than 10 times in recent 3 years), while their value is still low. Maybe it will make sense either marketing them more for bigger order volumes. Or reducing the number or purchases, keeping the same cumulative volume, to reduce the purchase and transportation costs.

```{r}
plot(est.pnbd, cumulative = FALSE)
```

The plot above shows the number of repeat transactions for customers in total. As can be observed, the repeat transactions were down during 2020 lock down period, which also influenced the prediction model. Nonetheless, the Pareto curve seems stable. 
Given up-to-current-date data, the model should be able to predict the customer repetitive transactions, as well as CLV, even more accurately.

```{r}
plot(est.pnbd, cumulative = TRUE)
```

On cumulative plot, the same picture can be observed. The repetition re-accurance trend was broken during the lockdown period. Thus, the current transactions follow the covid trend. Given the post-covid data for training, the model will be even more precise.
...
Mining the association rules with a support of at least 0.01, and a confidence of at least 0.8. There are slightly more than 10.000 transactions, this means that for the association rules mined the item combination occurs in at least 100 transactions. And a confidence level of 0.8 means that in 80% of the cases this leads to the item on the right hand. With these confidence and support settings, 359 association rules are mined. On the right-hand side the most dominant product categories are Daura, Turia, Lemon, Free, and Estrella, which will be dived into deeper in the following sections.
...
Since Daura is part of the more dominant product categories present in the previously mined association rules, the following code will mine rules specific to this product category.
...
The following table shows the top relevant association rules for the Daura product group. This means that any customer that buys the products on the left-hand side is very likely to buy a Daura product.
...
Since Turia is part of the more dominant product categories present in the previously mined association rules, the following code will mine rules specific to this product category.
...
The following table shows the top relevant association rules for the Turia product group. This means that any customer that buys the products on the left-hand side is very likely to buy a Turia product.
...
The following table shows the top relevant association rules for the Lemon product group. This means that any customer that buys the products on the left-hand side is very likely to buy a Lemon product.
...
Since Free is part of the more dominant product categories present in the previously mined association rules, the following code will mine rules specific to this product category.
...
The following table shows the top relevant association rules for the Free product group. This means that any customer that buys the products on the left-hand side is very likely to buy a Free product.
...
The following table shows the top relevant association rules for the Estrella product group. This means that any customer that buys the products on the left-hand side is very likely to buy a Estrella product.
...
+ The sales of the company follow a cyclical trend over the year, and this should be considered for doing the specific campaigns at different times of the annual calendar. It should be noted that COVID-19 had an impact on these sales. 
+ The company has a good market position in Catalonia, the Balearic Islands, and Rioja. This implies that it has a lot of potential for growth in the other fourteen Autonomous Communities. A deeper study of the consumption habits of those regions would be helpful.
+ Damm has different kinds of clients that deserve special attention. First, there are 67 that have a high expenditure level, but just until recently started purchasing (*Potential loyalist*). A strategy to consolidate this group is needed. In other categories like *Need attention* and *Others* the frequency is good but the recency is declining, then it is important to reactivate these clients.
+ Damm has 12% of the recent client base, which has done only one purchase in the last 3 years. This can be taken for consideration for a better focus on those client to increase their engagement with the business.
+ On the other hand, the company has clients that order frequently, while their total contribution to the revenue is not that efficient. 
+ Those can be observed in more details for business decisions follow-up, through the CLV model obtained in this analysis.
+ From the association rules analysis, we can extract that five product groups are dominant on the right-hand side for a good level of lift and confidence. These are: Daura, Turia, Lemon, Free and Estrella. A special marketing campaign can be created focusing on customers that mostly buy a product from one of these product groups.
...
* [12] INE, "Cifras oficiales del Padrón," [Online]. Available: https://www.ine.es/jaxiT3/Tabla.htm?t=2853. [Accessed 18 May 2022].
* [13] Statista, "Volumen de cerveza consumida por persona al año en España entre 2008 y 2020," 2022. [Online]. Available: https://es.statista.com/estadisticas/476791/consumo-per-capita-de-cerveza-en-espana/. [Accessed 18 May 2022].
