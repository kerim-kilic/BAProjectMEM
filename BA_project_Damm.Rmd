...
```{r}
##USA <- ggplot2::map_data('county', 'Massachusetts') %>% filter(long<-70.5 &long >-71.5 & lat>41.9&lat<42.6 &subregion!="barnstable" ) 

##p2 <- ggplot()+geom_polygon(data=USA, aes(x=long,y=lat, group=group)) + geom_point(data=boston, size=.2, aes(x=LON, y=LAT, color=MEDV)) + coord_equal(ratio=1) + labs(title="Median Value")
...
retailers_capitals <- merge(retailers, provinces, by.x = "region", by.y = "ï..PROVINCIA")

# Merge Orders, Retailers and Provinces
orders_retailers_capitals <- merge(orders_prices, provinces, by.x = "region", by.y = "ï..PROVINCIA")
...
ggplot(Spain) + geom_sf(fill = "#00AA88", color = "#ffffff") +
  geom_point(data = provinces, aes(x = Longitud, y = Latitud))
