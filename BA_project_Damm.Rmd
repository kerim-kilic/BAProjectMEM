...
Looking through the category data we see a lot of NA values, and after looking through the types of the categories we realize that those are just the 'regular' retailers, so we will just replace NAs with type 'Regular'.
```{r}
tabyl(retailers$category)
retailers$category <- ifelse(is.na(retailers$category), "Regular", retailers$category)
```
