...
The software used for the development of the study and the writing of the report is. The first step is to define the work directory and to load the libraries: 
...
In this section, the table quantity will be cleaned. First,  the clean_data() function is run to clean the names of the variables, remove the empty rows and columns, remove the columns with only a constant value, remove the duplicate values, and remove the column x since it does not give any important information.
...
In order to perform the analysis, the tables orders and prices should be merged. This is done using the merge function, and doing it by the "material" variable. In order to have relevant data, this data is filter from 2002 on wards. 
...
Looking through the category data a lot of NA values can be seen, and after checking with the company, it was realized that they are not classified yet. Thus, the will be marked so for now and classify them afterwards as part of the work. 
...
## Oreder lines per product type
First, the  number of times a type of article appears in an order is counted, and from this, a graph taking into account the percentage is made.  
...
##Top clients 
Now, the clients with more order lines will be shown in a table. 
```{r}
tab_1 <- orders_prices %>%
...
Also, the clients with only 1 order line will be studdied. 
...
```
It can be seen that more than 30% of the order lines come from Barcelona, whereas less than 2% come from Vizcaya. 
...
orders_retailers_capitals <- orders_retailers_capitals %>% distinct(
  id_establishment,material, date ,quantity, price, Capital, Latitude, Longitude, Autonomous_community ,Population_community,Population, price_liter, Beer_consumption)
...
Revenue_region <- orders_retailers_capitals_year %>% group_by(Capital) %>%
  summarise(Sales = sum(Revenue), Volume = sum(quantity), Population_community = mean(Population_community),
            Longitude = mean(Longitude), Latitude = mean(Latitude))

Revenue_region <- Revenue_region %>% mutate(sales_per_capita = Sales / Population_community)
Revenue_region <- Revenue_region %>% mutate(Volume_per_capita = Volume / Population_community)
```
...
# Consumption-Sales Autonomous Community

```{r}
#Summary autonomous community
Revenue_Autonomous_community <- orders_retailers_capitals_year %>%
  group_by(Autonomous_community) %>% 
  summarise(Sales = sum(Revenue), Volume = sum(quantity), Population_community = mean(Population_community), Beer_consumption_per_capita = mean(Beer_consumption),
            Longitude = mean(Longitude), Latitude = mean(Latitude))

Revenue_Autonomous_community <- Revenue_Autonomous_community %>%
  mutate(Sales_per_capita = Sales / Population_community, Volume_per_capita = Volume / Population_community, Damm_market_percentage = Sales_per_capita/Beer_consumption_per_capita)

Spain_ccaa <- esp_get_ccaa()

ggplot(Spain_ccaa) + geom_sf() + geom_point(data = Revenue_Autonomous_community,
                                            aes(x = Longitude, y = Latitude, size = Volume_per_capita))

ggplot(Spain_ccaa) + geom_sf() + geom_point(data = Revenue_Autonomous_community,
                                            aes(x = Longitude, y = Latitude, size = Beer_consumption_per_capita))
...
Show top 25 of mined rules based on lift

```{r}
top_rules <- rules_df %>% head(25)
